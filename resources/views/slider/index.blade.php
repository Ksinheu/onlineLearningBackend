                ...
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title text-primary" id="uploadModalLabel">បញ្ចូលរូបភាពស្លាយ</h5>
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach ($sliders as $index => $slider)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td><img src="{{ Storage::url($slider->image) }}" alt="" width="50px"
                                        height="50px">
                                </td>
                                <td><span
                                        class="p-2 badge {{ $slider->status === 'active' ? 'bg-success' : 'bg-warning' }}">{{ ucfirst($slider->status) }}</span>
                                </td>
                                <td>
                                    <a href="{{ route('slider.show', $slider->id) }}" class="btn btn-warning"
                                        data-bs-toggle="modal" data-bs-target="#showModel{{ $slider->id }}"><i
                                            class="fa-solid fa-eye"></i></a>
                                    <a href="{{ route('slider.edit', $slider->id) }}" class="btn btn-info"
                                        data-bs-toggle="modal" data-bs-target="#updateModel{{ $slider->id }}"><i
                                    ...
                                    <form action="{{ route('slider.destroy', $slider->id) }}" method="POST"
                                        ...
                                        <button type="submit" class="btn btn-danger btn-deleted"
                                            data-id="{{ $slider->id }}" data-name="{{ $slider->image }}"><i
                                                class="fa-solid fa-trash"></i></button>
                                    </form>
                                </td>
                            </tr>
                            <div class="modal fade" id="updateModel{{ $slider->id }}" tabindex="-1"
                                aria-labelledby="uploadModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title text-primary" id="uploadModalLabel">កែប្រែរូបភាពស្លាយ
                                            </h5>
                                            ...
                                            <form action="{{ route('slider.update', $slider->id) }}" method="POST"
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <select name="status" id="status" class="form-select" required>
                                                        <option value="">-- សូមជ្រើសរើសស្ថានភាព --</option>
                                                        <option value="active"
                                                            {{ $slider->status == 'active' ? 'selected' : '' }}>Active
                                                        </option>
                            ...
                            <div class="modal fade" id="showModel{{ $slider->id }}" tabindex="-1"
                                aria-labelledby="uploadModalLabel" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title text-primary" id="uploadModalLabel">មើលរូបស្លាយ</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body">
