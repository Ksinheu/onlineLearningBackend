...
@section('content')
    <div class="col-md-12 pb-3 home-content">
                        ...
                        <tbody>
                            @foreach ($exercise as $index => $ex)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('exercise.edit', $ex->id) }}" class="btn btn-info" data-bs-toggle="modal"
        ...
        <div class="mb-3">
            ...
            <select name="lesson_id" id="lesson_id" class="form-control">
                @foreach($lesson as $lessions)
                    <option value="{{ $lessions->id }}" {{ $lessions->id == $ex->lesson_id ? 'selected' : '' }}>
                        {{ $lessions->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            <label for="exercise_element" class="form-label">Exercise Element</label>
            <input type="text" name="exercise_element" id="exercise_element" class="form-control" value="{{ old('exercise_element', $ex->exercise_element) }}">
        ...
        <!-- Submit Button -->
        <button type="submit" class="btn btn-primary"><i class="fa-solid fa-pen-to-square"></i> កែប្រែ</button>
    </form>
                    </div>
                </div>
            </div>
